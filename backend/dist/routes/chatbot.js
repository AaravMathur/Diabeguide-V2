...
const getLatestReadingEvaluation = (recentReadings) => {
...
const getFallbackAIResponse = (question, recentReadings = []) => {
...
router.post("/sessions/:id/message", async (req, res) => {
    ...
    try {
        ...
        let readingsContext = "";
        if (recentReadings && recentReadings.length > 0) {
            readingsContext = recentReadings.map(r => `- Date: ${r.date}, Time: ${r.time} (${r.timeOfDay}), Level: ${r.value} mg/dL, Meal: ${r.meal}`).join("\n");
        }
        else {
